...
{ 
   u8 Res;
	...
	if(( UART1_RX_NUM&0x80)==0)/*接收未完成*/
	{
	    if( UART1_RX_NUM&0x40)/*接收到了0x0d*/
		{
		  if(Res!=0x0a) UART1_RX_NUM=0;/*接收错误,重新开始*/
		  else  UART1_RX_NUM|=0x80;	/*接收完成了 */
		}
            else /*还没收到0X0D*/
              {	
...
__interrupt void UART3_RX_IRQHandler(void)
{
   ...
   u8 Res;
	...
	Res =UART3_ReceiveData8();
        /*(USART1->DR);读取接收到的数据,当读完数据后自动取消RXNE的中断标志位*/
	if(( UART3_RX_NUM&0x80)==0)/*接收未完成*/
	{
	    if( UART3_RX_NUM&0x40)/*接收到了0x0d*/
		{
		  if(Res!=0x0a) UART3_RX_NUM=0;/*接收错误,重新开始*/
		  else  UART3_RX_NUM|=0x80;	/*接收完成了 */
		}
            else /*还没收到0X0D*/
              {	
                if(Res==0x0d) UART3_RX_NUM|=0x40;
                else
                  {
                    RxBuffer3[ UART3_RX_NUM&0X3F]=Res ;
                     UART3_RX_NUM++;
                      if( UART3_RX_NUM>63) UART3_RX_NUM=0;/*接收数据错误,重新开始接收*/  
                  }		 
	  }
	}  		 
		}
